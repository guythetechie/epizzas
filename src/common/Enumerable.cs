...
//{
//    public static IO<Unit> IterParallel<T>(this Seq<T> enumerable, Func<T, IO<Unit>> action) =>
//        enumerable.Traverse(action).as
//        enumerable.AsEnumerableM()
//                  .Traverse(action)
//                  .Map(_ => Prelude.unit)
//                  .As();
...
//    {
//        IO<Unit> getIo(T t) => IO.liftAsync(async () =>
//        {
//            await action(t, cancellationToken);
//            return Prelude.unit;
//        });

//        var b = enumerable.AsEnumerableM().Traverse(getIo).As();
...
//        dictionary.AsEnumerableM()
//                  .Choose(kvp => f(kvp.Value).Map(value2 => KeyValuePair.Create(kvp.Key, value2)))
//                  .ToImmutableDictionary();
//}
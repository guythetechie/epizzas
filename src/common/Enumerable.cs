...
public static class IEnumerableExtensions
{
    ...
    /// <summary>
    /// Iterates over an <seealso cref="IEnumerable{T}"/> and executes <paramref name="action"/> for each element.
    /// Each action is executed in parallel with based on <paramref name="maxDegreeOfParallelism"/>. The function will wait for all actions to complete before returning.
    /// </summary>
    ...
    /// Each action is executed sequentially. The function returns after all actions have executed.
    /// </summary>
    public static async ValueTask Iter<T>(this IEnumerable<T> enumerable, Func<T, CancellationToken, ValueTask> action, CancellationToken cancellationToken) =>
...
public static class IAsyncEnumerableExtensions
{
    ...
    public static async ValueTask<FrozenDictionary<TKey, T>> ToFrozenDictionary<TKey, T>(this IAsyncEnumerable<T> enumerable, Func<T, TKey> keySelector, CancellationToken cancellationToken, IEqualityComparer<TKey>? comparer = default) where TKey : notnull =>
    ...
    public static IAsyncEnumerable<T[]> Chunk<T>(this IAsyncEnumerable<T> source, int size, CancellationToken cancellationToken)
    {
    ...
    private static async IAsyncEnumerable<T[]> GetChunkIterator<T>(this IAsyncEnumerable<T> source, int size, [EnumeratorCancellation] CancellationToken cancellationToken)
    {
        ArgumentOutOfRangeException.ThrowIfNegativeOrZero(size);

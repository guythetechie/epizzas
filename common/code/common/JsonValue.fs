...
let asAbsoluteUri jsonValue =
    let error = JsonError.fromMessage "JSON value is not an absolute URI."

    monad {
        let! stringValue = asString jsonValue |> JsonResult.mapError (konst error)
        ...
        | true, uri when
            ...
            ->
            return uri
        | _ -> return! JsonResult.fail error
    }

let asGuid jsonValue =
    let error = JsonError.fromMessage "JSON value is not a GUID."
...
let asDateTimeOffset (jsonValue: JsonValue) =
    let error = JsonError.fromMessage "JSON value is not a date time offset."

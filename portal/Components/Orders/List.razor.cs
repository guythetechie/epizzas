...
public partial class List(ListOrders listOrders) : ComponentBase
{
    ...
    private sealed record GridItem
    {
        ...
        {
            var id = OrderId.toString(order.Id);
            var (status, lastModifiedBy, lastModifiedOn) = order.Status switch
            {
                OrderStatus.Created created =>
                    (nameof(OrderStatus.Created), created.Item.By, created.Item.Date),
                OrderStatus.Cancelled cancelled =>
                    (nameof(OrderStatus.Cancelled), cancelled.Item.By, cancelled.Item.Date),

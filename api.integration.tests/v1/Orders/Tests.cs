...
namespace api.integration.tests.v1.Orders;

internal delegate ValueTask SetUpDatabase(CancellationToken cancellationToken);
...
internal static class TestModule
{
    ...
    {
        ConfigureSetUpDatabase(builder);
        HttpModule.ConfigureHttpClient(builder);
    ...
    {
        var setUpDatabase = provider.GetRequiredService<SetUpDatabase>();
        var clientFactory = provider.GetRequiredService<IHttpClientFactory>();
        var generator = Generator.Order;

        return async cancellationToken =>
        {
            await setUpDatabase(cancellationToken);
            await generator.SampleAsync(async order =>
            {
        ...
        Eff<Option<(Order Order, ETag ETag)>> findOrder(OrderId orderId) =>
            ...
            from option in response.StatusCode switch
            {
                ...
                _ => from _ in liftEff(response.EnsureSuccessStatusCode)
                     ...
                     from order in (from order in Order.Deserialize(jsonObjectWithId)
                                    from eTagString in jsonObject.GetStringProperty("eTag")
                                    let eTag = new ETag(eTagString)
                                    select (order, eTag)).ToEff()
                     select Some(order)
            }
            select option;
        ...
        Eff<ImmutableArray<(Order Order, ETag ETag)>> listOrders() =>
            ...
            from jsonObject in getJsonObjectFromResponse(response)
            from orders in (from value in jsonObject.GetJsonArrayProperty("value")
                            from orders in value.AsIterable()
                                                .Traverse(node => from resultJson in node.AsJsonObject()
                                                                  from order in Order.Deserialize(resultJson)
                                                                  from eTagString in resultJson.GetStringProperty("eTag")
                                                                  let eTag = new ETag(eTagString)
                                                                  select (order, eTag))
                            select orders.ToImmutableArray()).ToEff()
            select orders;
        ...
        static Gen<JsonNode?> invalidateOrder(JsonNode? order) =>
            ...
            {
                JsonObject jsonObject => Gen.OneOf<JsonNode?>(Gen.Const(jsonObject.RemoveProperty("pizzas")),
                                                                        from invalidPizzas in jsonObject.GetProperty("pizzas")
                                                                                                        .Match(invalidatePizzas,
                                                                                                               _ => invalidatePizzas(null))
        ...
        static Gen<JsonNode?> invalidatePizza(JsonNode? pizza) =>
            ...
            {
                JsonObject jsonObject => Gen.OneOf(JsonNodeGenerator.NonJsonObject.Null(),
                                                   from p in Gen.Const(jsonObject)
                                                   select p.RemoveProperty("size"),
                                                   from invalidSize in jsonObject.GetProperty("size")
                                                                                 .Match(invalidatePizzaSize,
                                                                                        _ => invalidatePizzaSize(null))
                                                   select jsonObject.SetProperty("size", invalidSize),
                                                   from p in Gen.Const(jsonObject)
                                                   select p.RemoveProperty("toppings"),
                                                   from invalidToppings in jsonObject.GetProperty("toppings")
                                                                                     .Match(invalidatePizzaToppings,
                                                                                            _ => invalidatePizzaToppings(null))
        ...
        static Gen<JsonNode?> invalidatePizzaSize(JsonNode? size) =>
            from invalidSize in Gen.OneOf(JsonNodeGenerator.NonJsonValue.Null(),
                                          JsonValueGenerator.NonString.Null(),
                                          from value in JsonValueGenerator.String
                                          where value.ToString() is not (nameof(PizzaSize.Small) or nameof(PizzaSize.Medium) or nameof(PizzaSize.Large))
                                          select value)
            select invalidSize;
        ...
        static Gen<JsonNode?> invalidatePizzaTopping(JsonNode? topping) =>
            ...
            {
                JsonObject toppingObject => from invalidTopping in Gen.OneOf(JsonNodeGenerator.NonJsonObject.Null(),
                                                                            from t in Gen.Const(toppingObject)
                                                                            select t.RemoveProperty("kind"),
                                                                            from invalidKind in toppingObject.GetProperty("kind")
                                                                                                             .Match(invalidatePizzaToppingKind,
                                                                                                                    _ => invalidatePizzaToppingKind(null))
                                                                            select toppingObject.SetProperty("kind", invalidKind),
                                                                            from t in Gen.Const(toppingObject)
                                                                            select t.RemoveProperty("amount"),
                                                                            from invalidAmount in toppingObject.GetProperty("amount")
                                                                                                               .Match(invalidatePizzaToppingAmount,
                                                                                                                      _ => invalidatePizzaToppingAmount(null))
        ...
        static Gen<JsonNode?> invalidatePizzaToppingKind(JsonNode? kind) =>
            from invalidKind in Gen.OneOf(JsonNodeGenerator.NonJsonValue.Null(),
                                          JsonValueGenerator.NonString.Null(),
                                          from value in JsonValueGenerator.String
                                          where value.ToString() is not (nameof(PizzaToppingKind.Cheese) or nameof(PizzaToppingKind.Pepperoni) or nameof(PizzaToppingKind.Sausage))
                                          select value)
            select invalidKind;

        static Gen<JsonNode?> invalidatePizzaToppingAmount(JsonNode? amount) =>
            from invalidAmount in Gen.OneOf(JsonNodeGenerator.NonJsonValue.Null(),
                                            JsonValueGenerator.NonString.Null(),
                                            from value in JsonValueGenerator.String
                                            where value.ToString() is not (nameof(PizzaToppingAmount.Light) or nameof(PizzaToppingAmount.Normal) or nameof(PizzaToppingAmount.Extra))
                                            select value)
    ...
    private static SetUpDatabase GetSetUpDatabase(IServiceProvider provider)
    {
        ...
        return async cancellationToken =>
        {
            ...
            Container container = await database.CreateContainerIfNotExistsAsync(containerName, "/orderId", cancellationToken: cancellationToken);
            ...
            var action = from results in CosmosModule.GetQueryResults(container, queryOptions)
                         from _ in results.AsIterable()
                                          .Traverse(result => from id in CosmosModule.GetCosmosId(result)
                                          ...
                                          .Traverse(results => results.Traverse(x => CosmosModule.DeleteRecord(container, x.id, x.partitionKey, x.eTag)))
                                          .As()
                         select Unit.Default;
